function [u_t] = topolgy_LIF(u,u_af,z,M,N)
...
% u=0.01 foreground, u=0.99 background

u1 = u;
u1(u > 0.5) = 1; % Replace 0.99 with 1 (for numerical stability)
u1(u < 0.5) = 0; % Replace 0.01 with 0 (for numerical stability)
u1_af = u_af;
u1_af(u_af > 0.5) = 1; % Replace 0.99 with 1
u1_af(u_af < 0.5) = 0; % Replace 0.01 with 0
u2 = 1-u1;
u2_af = 1-u1_af;
X = find(u1-u1_af==1);
s = zeros(length(X),1);
...
u_t = u1;
u_t(u1 == 1) = 0.99; % Replace 1 with 0.99
u_t(u1 == 0) = 0.01; % Replace 0 with 0.01
